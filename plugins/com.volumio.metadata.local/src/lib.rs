...
pub const PLUGIN_NAME: &str = "com.volumio.metadata.local";
...
pub fn manifest() -> Manifest {
    Manifest::from_toml(MANIFEST_TOML)
        .expect("com-volumio-metadata-local: embedded manifest must parse")
}
...
impl Respondent for MetadataLocalPlugin {
    fn handle_request<'a>(
        ...
        async move {
            ...
            let body = out
                .json_bytes()
                .map_err(|e| PluginError::Permanent(format!("metadata response JSON: {e}")))?;
