...
**Plugin:** `com.volumio.metadata.local`  
...
Optional TOML under the plugin config path, e.g. `plugins.d/com.volumio.metadata.local.toml` (see manifest `prerequisites`):
...
Paths must be **absolute**; they resolve relative `mpd-path` `target.value` the same way as `com.volumio.artwork.local` and the MPD warden’s `file` field.
