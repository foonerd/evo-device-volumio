...
//!
//! The steward reads `/etc/evo/plugins.d/com.volumio.playback.mpd.toml`
...
pub(crate) enum ConfigError {
    ...
    #[error("timeouts.{field} must be in {min}..={max} milliseconds, got {value}")]
    TimeoutOutOfRange {
        field: &'static str,
        value: i64,
        min: u64,
...
fn parse_endpoint(value: Option<&toml::Value>) -> Result<MpdEndpoint, ConfigError> {
...
fn parse_timeouts(value: Option<&toml::Value>) -> Result<ConnectTimeouts, ConfigError> {
    let Some(value) = value else {
