    ...
    W: AsyncWrite + Unpin + Send,
{
    ...
    pub(crate) async fn read_line_with_timeout(
        ...
        self.scratch.clear();
        let outcome = time::timeout(budget, self.reader.read_line(&mut self.scratch)).await;
        let n = match outcome {
            Ok(Ok(n)) => n,
    ...
    pub(crate) async fn write_all_with_timeout(
        ...
        match time::timeout(budget, write_and_flush).await {
            Ok(Ok(())) => Ok(()),
            Ok(Err(e)) => Err(MpdError::Transport(TransportError::Io { source: e })),
...
fn classify_read_error(e: io::Error) -> MpdError {
    match e.kind() {
        ...
        io::ErrorKind::UnexpectedEof => MpdError::Transport(TransportError::Closed),
