...
pub(crate) fn classify_line(line: &str) -> Result<ClassifiedLine, ProtocolError> {
    if line == "OK" {
...
mod tests {
    ...
    fn serialise_command_escapes_double_quote() {
        let bytes = serialise_command("find", &["Artist", "say \"hi\""]).unwrap();
    ...
    fn serialise_command_preserves_utf8_arg() {
        let bytes = serialise_command("find", &["Artist", "Bj\u{00f6}rk"]).unwrap();
