...
//! `com.volumio.playback.mpd` (compound `"{artist}|{album}"` / empty artist →
...
const AUDIO_EXTS: &[&str] = &[
    "flac", "mp3", "m4a", "mp4", "m4b", "aac", "ogg", "oga", "opus", "wma", "wav", "aif", "aiff",
    "wv", "ape", "mpc", "mka", "webm", "3gp", "aax",
];
...
/// Parse `value` to match `com.volumio.playback.mpd` / album subjects:
/// `splitn(2, '|')` → left = artist (empty or whitespace → [`UNKNOWN_ARTIST`]),
/// right = album (required, non-empty after trim).
pub fn parse_mpd_album_value(value: &str) -> Result<(String, String), ParseError> {
    let v = value.trim();
...
pub fn first_matching_audio_path(
    ...
    for root in library_roots {
        if let Some(p) = scan(root.as_path(), want_artist, want_album, &mut examined)? {
            return Ok(Some(p));
        }
    }
    Ok(None)
...
fn scan(
    ...
    let mut names = read_dir_names(path)
        .map_err(|e| MatchError::Io(format!("read_dir {}: {e}", path.display())))?;
    names.sort();
    for name in names {
        if name.starts_with('.') {
            continue;
...
fn audio_file_matches(path: &Path, want_artist: &str, want_album: &str) -> bool {
    let tagged = match read_from_path(path) {
        Ok(t) => t,
        Err(_) => return false,
    };
    if let Some(tag) = tagged.primary_tag().or_else(|| tagged.first_tag()) {
        return file_tag_matches(tag.artist(), tag.album(), want_artist, want_album);
    }
    false
}
...
mod tests {
    ...
    fn end_to_end_finds_in_tree() {
        ...
        let found = first_matching_audio_path(&[dir.path().to_path_buf()], &a, &al).unwrap();
